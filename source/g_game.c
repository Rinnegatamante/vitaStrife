...
void G_DoLoadGame (boolean userload) 
{
	...
	char flname[256];
	sprintf(flname, "ux0:/data/vitaStrife/%s", loadpath);
    save_stream = fopen(flname, "rb");
...
{ 
	...
	char flname[256];
	sprintf(flname, "ux0:/data/vitaStrife/%s", temp_savegame_file);
    save_stream = fopen(flname, "wb");

    in = fopen(flname, "rb" );					// FOR PSP
...
//    fclose(save_stream);							// DISABLED FOR PSP
	...
	sprintf(flname, "ux0:/data/vitaStrife/%s", savegame_file);
    sceIoRemove(flname);

    // Now open the destination savegame file for writing

    out= fopen(flname, "wb" );						// FOR PSP
