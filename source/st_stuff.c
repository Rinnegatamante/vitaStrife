...
boolean			    haskeyinrange/* = false*/;	// MOVED HERE FOR PSP
...
//  intercept cheats.
boolean ST_Responder(event_t* ev)		// FOR PSP: MOST STUFF HERE HAS BEEN CHANGED TO ALLOW...
{						// ...CONTROLLING NEARLY EVERYTHING USING ONLY 1 BUTTON
...
*/
        {
            if(ev->data1 != key_mission)// || netgame)
            {
                if(ev->data1 ==  key_invpop)
                {
		    if(!automapactive)			// ADDED CONDITION FOR PSP
		    {
/*
    	                st_keypage = -1;
...
*/
	...
	else if(plyr->inventorycursor == plyr->numinventory - 1)	// ADDED FOR PSP (SAVES A KEY)
...
{
    if(!num)
        V_DrawPatch(x, y, 0, invfonty[0]);
    
    while(num)
    {
        V_DrawPatch(x, y, 0, invfonty[num % 10]);
...
void ST_drawLine(int x, int y, int len, int color)
{
    ...
    byte *drawpos = screens[0] + y * SCREENWIDTH + x;
    int i = 0;
...
void ST_doRefresh(void)
{
    ...
    if (st_statusbaron)
    {
        ...
        V_DrawPatch(ST_X, ST_Y, 0, invback);
...
*/
            ...
            num_x += 35, icon_x += 35, i++, numdrawn++)
        {
            ...
            V_DrawPatch(icon_x, 182, 0, patch);
            ST_drawNumFontY(num_x, 191, plyr->inventory[i].amount);
        }

        // haleyjd 20100919: Draw sigil icon
        if(plyr->weaponowned[wp_sigil])
            V_DrawPatch(253, 175, 0, invsigil[plyr->sigiltype]);

        // haleyjd 20100919: Draw ammo
        if(st_lastammo < NUMAMMO)
            V_DrawPatch(290, 180, 0, invammo[st_lastammo]);

        // haleyjd 20100919: Draw armor
        if(plyr->armortype)
        {
            V_DrawPatch(2, 177, 0, invarmor[plyr->armortype - 1]);
...
*/
    {
        ...
        for(; keycount < ST_KEYSPERPAGE && key < NUMCARDS; ++key, ++keycount, ++info)
        {
	    	...
	    	if(st_keypage > 1 && keycount > 0)		// FOR PSP
            	    haskeyinrange = false;
	    	else
            	    haskeyinrange = true;			// FOR PSP
            }
...
//    screens[4] = Z_Malloc(SCREENWIDTH*(32+1), PU_STATIC, 0);
