...
( player_t*	player,
  pspdef_t*	psp )
{	
//    if(player->readyweapon != player->pendingweapon)	<----	// FOR PSP: ('IF' CONDITION IMPORTED):
    	psp->sy += LOWERSPEED;					// DON'T LOWER THE WEAPON IF WE...
								// ...ONLY GO THE FISTS FOR FIGHTING
    // Is already down.						// (ONLY LOWER IN ANY OTHER CASE)
    if (psp->sy < WEAPONBOTTOM )				// NOPE: NOT WORKING AS EXPECTED
        return;
