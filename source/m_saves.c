...
void ClearTmp(void)
{
	...
	char dname[256];
	sprintf(dname, "ux0:/data/vitaStrife/%s", savepathtemp);
    if(!(sp2dir = opendir(dname)))
    ...
    while((f = readdir(sp2dir)))
    {
		...
		char flname[256];
		sprintf(flname, "ux0:/data/vitaStrife/%s", filepath);
		sceIoRemove(flname);
...
void ClearSlot(void)
{
	...
	char dname[256];
	sprintf(dname, "ux0:/data/vitaStrife/%s", savepath);
    if(!(spdir = opendir(dname)))
...
void M_SaveMoveMapToHere(void)
{
    ...
    {
		char flname[256], flname2[256];
		sprintf(flname, "ux0:/data/vitaStrife/%s", heresave);
		sprintf(flname2, "ux0:/data/vitaStrife/%s", mapsave);
        sceIoRemove(flname);
        sceIoRename(flname2, flname);
    }
...
void M_ReadMisObj(void)
{
	...
	char flname[256];
	sprintf(flname, "ux0:/data/vitaStrife/%s", srcpath);
    if((f = fopen(flname, "rb")))
    {
